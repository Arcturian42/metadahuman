...
- 2026-07-18 — Accessibility pass on the multi-step form (`src/app/form/page.tsx`): added `htmlFor`/`id` pairings, `aria-invalid`/`aria-describedby` error linking, visible focus rings, `aria-label="Back"` on the icon back button, and `useReducedMotion` handling for Framer Motion step transitions. Typecheck, lint, and 59 tests pass.
